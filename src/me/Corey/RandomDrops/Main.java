...
public class Main extends JavaPlugin implements Listener {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if (label.equalsIgnoreCase("RandomDrops")) {
			if (!sender.hasPermission("randomdrops.reload")) {
				sender.sendMessage(ChatColor.RED + "You do not have access to this command");
				return true;
			}
			if (args.length == 0) {
				// /randomdrops
				sender.sendMessage(ChatColor.RED + "Usage: /randomdrops reload");
				return true;
			}
			if (args.length > 0) {
				// /randomdrops reload
		...
		this.getConfig().getConfigurationSection("blocks").getKeys(false).forEach(key -> {
			if (key.equalsIgnoreCase(event.getBlock().getType().toString())) {
				...
				for (String i : this.getConfig().getStringList("blocks." + key)) {
					// DIRT DIR
					try {
					item = new ItemStack(Material.matchMaterial(i), r.nextInt(1) + 1);
				} catch (Exception e) {
